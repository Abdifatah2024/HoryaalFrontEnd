...
import {
  FiUsers, FiCalendar, FiBook, FiUser,
  FiBarChart2, FiBell, FiMail,
  FiPieChart, FiPlus, FiChevronRight, FiSettings, FiUpload
} from 'react-icons/fi';
import { HiOutlineAcademicCap } from 'react-icons/hi';

// Define Interfaces for better type checking (if using TypeScript)
// Even if you're using JavaScript, these comments help understand data structure
interface JwtPayload {
  role: string;
  exp: number;
}
...
const Dashboard = () => {
  ...
  try {
    ...
    // navigate('/login'); // Uncomment if you want to redirect
    return <div className="text-center text-red-600 font-bold p-10">Invalid token format. Please re-login.</div>;
  }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [studentsRes, employeesRes, absentRes, incomeRes, withBusRes, withoutBusRes] = await Promise.all([
          axios.get("http://localhost:4000/student/studentList", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:4000/user/employees", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:4000/student/attendance/absent-today", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:4000/fee/income/today", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:4000/student/students/with-bus", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:4000/student/students/without-bus", { headers: { Authorization: `Bearer ${token}` } })
        ]);
    ...
    const fetchMonthlyData = async () => {
      try {
        setLoading(prev => ({ ...prev, monthlyIncome: true, classPayment: true }));
        const [monthlyIncomeRes, classPaymentRes] = await Promise.all([
          axios.get(`http://localhost:4000/fee/income-required?month=${month}&year=${year}`, { headers: { Authorization: `Bearer ${token}` } }),
          axios.get(`http://localhost:4000/fee/Classfee/status?month=${month}&year=${year}`, { headers: { Authorization: `Bearer ${token}` } })
        ]);
  ...
  const stats = [
    {
      ...
      details: [
        {
          label: "Male",
          value: maleCount !== null && totalStudents !== null && totalStudents > 0 ? `${maleCount} (${((maleCount / totalStudents) * 100).toFixed(1)}%)` : "--",
          color: "text-blue-600",
          icon: <FiUser className="text-blue-600" size={14} />
        },
        {
          label: "Female",
          value: femaleCount !== null && totalStudents !== null && totalStudents > 0 ? `${femaleCount} (${((femaleCount / totalStudents) * 100).toFixed(1)}%)` : "--",
          color: "text-pink-600",
          icon: <FiUser className="text-pink-600" size={14} />
        },
        {
    ...
    <div className="p-6 min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        ...
        <div className="flex gap-3 mt-4 md:mt-0">
          ...
          <button className="relative p-2 rounded-lg bg-white shadow-sm hover:shadow-md text-indigo-600 transition-all">
            <FiBell size={20} />
            <span className="absolute top-0 right-0 h-2 w-2 rounded-full bg-red-500"></span> {/* Notification dot */}
          </button>
          {/* Mail Button */}
          <button className="p-2 rounded-lg bg-white shadow-sm hover:shadow-md text-indigo-600 transition-all">
            <FiMail size={20} />
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <button
            ...
            className="group bg-white hover:bg-green-600 p-4 rounded-lg border border-green-100 shadow-sm transition-all flex items-center justify-between hover:shadow-md"
          >
            <div className="flex items-center gap-3">
              <div className="bg-green-100 p-2 rounded-lg text-green-600 group-hover:bg-white group-hover:text-green-600 transition-colors">
                <FiUpload size={18} />
              </div>
              <span className="font-medium text-gray-700 group-hover:text-white">Import Data</span>
            </div>
            <FiChevronRight className="text-gray-400 group-hover:text-white" />
          </button>
        </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-lg hover:shadow-xl transition-shadow">
        ...
        {loading.classPayment ? (
          ...
          <>
            ...
            {showDetails && classPaymentData.students.length === 0 && (
              <div className="text-center py-5 text-gray-500 bg-gray-50 rounded-lg mt-4">
                No student details available for this month.
              </div>
            )}
          </>
        ) : (
          <div className="text-center py-10 text-gray-500 bg-gray-50 rounded-lg">
            No class payment summary available for {dayjs().month(month - 1).format("MMMM YYYY")}.
          </div>
        )}
      </div>
    </div>
