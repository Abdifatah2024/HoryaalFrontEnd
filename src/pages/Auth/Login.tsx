...
//   localStorage.setItem("justLoggedIn", "true"); // ✅ flag set here
...
const Login = () => {
  ...
  useEffect(() => {
    if (lockTimer && lockTimer > 0) {
      ...
      setLockTimer(null);
      toast.success("Account unlocked! Please try again.", { id: "login" });
    }
  }, [lockTimer]);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (lockTimer) {
        toast.error(`Please wait ${lockTimer} seconds before trying again.`, {
          id: "lockout_warning",
        });
        return;
      }
      ...
      >
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={!!lockTimer || loading}
                placeholder="you@example.com"
                className={`w-full pl-10 pr-3 py-3 border rounded-lg transition duration-200 ease-in-out
                  focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500
                  ${formik.touched.email && formik.errors.email ? "border-red-500" : "border-gray-300"}
                  ${!!lockTimer || loading ? "bg-gray-50 cursor-not-allowed" : "bg-white"}
                  text-gray-900 caret-gray-900 placeholder-gray-400
                  dark:bg-gray-900 dark:text-gray-100 dark:caret-gray-100 dark:placeholder-gray-400
                `}
              />
            </div>
            {formik.touched.email && formik.errors.email && (
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={!!lockTimer || loading}
                placeholder="Enter your password"
                className={`w-full pl-10 pr-10 py-3 border rounded-lg transition duration-200 ease-in-out
                  focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500
                  ${formik.touched.password && formik.errors.password ? "border-red-500" : "border-gray-300"}
                  ${!!lockTimer || loading ? "bg-gray-50 cursor-not-allowed" : "bg-white"}
                  text-gray-900 caret-gray-900 placeholder-gray-400
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.99 }}
            disabled={loading || !!lockTimer || !formik.isValid}
            className={`w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-lg shadow-md text-base font-semibold text-white transition-all duration-200 ease-in-out
              ${loading || !!lockTimer || !formik.isValid
          ...
          >
            {loading ? (
              ...
              <>
                Sign In <FiLogIn className="ml-2 h-5 w-5" />
              </>
            )}
          </motion.button>
        </form>
        ...
        {lockTimer !== null && (
          ...
          >
            <FiAlertCircle className="h-5 w-5 mr-2" />
            Account locked. Try again in {lockTimer} second{lockTimer !== 1 ? "s" : ""}.
          </motion.div>
        )}
        ...
        <div className="text-center text-sm text-gray-600 mt-7">
          ...
          <button
            onClick={() =>
              toast.success("Please contact your administrator for account creation.")
            }
            className="font-semibold text-indigo-600 hover:text-indigo-800 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 rounded"
          >
            Contact administrator
