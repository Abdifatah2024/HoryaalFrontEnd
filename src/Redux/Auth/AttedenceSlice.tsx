...
export const markAttendance = createAsyncThunk(
  ...
  async (
    ...
    try {
      const response = await axios.post(
        `${BASE_API_URL}/student/createattendance`, // ✅ Fixed typo
        { studentId, present, remark },
        { headers: { Authorization: `Bearer ${Access_token}` } }
      );
      return response.data;
...
export const fetchAttendanceRecords = createAsyncThunk(
  ...
  async (
    ...
    try {
      const response = await axios.get(
        `${BASE_API_URL}/student/attendancelist/${studentId}?${queryParams.toString()}`, // ✅ Fixed typo
        { headers: { Authorization: `Bearer ${Access_token}` } }
      );
      return response.data;
    } catch (error) {
...
const attendanceSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateAttendanceRecord.fulfilled, (state, action) => {
        const updatedAttendance = action.payload?.attendance ?? action.payload;
        state.updateStatus = 'succeeded';
