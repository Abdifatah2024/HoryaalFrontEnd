...
interface CreateOrUpdateExpenseResponse {
  expense: Expense;
  financialSummary?: FinancialSummary;
}

interface MonthlyBalanceResponse extends FinancialSummary {
  month: number;
  year: number;
}
...
interface State {
  ...
  totalCount: number;
  incomeSummary: FinancialSummary | null;
  expenseCategorySummary?: ExpenseSummaryResponse | null;
}
...
const expenseSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getMonthlyBalance.fulfilled, (state, action) => {
        state.incomeSummary = action.payload || null;
        state.error = null;
        state.message = "Monthly balance loaded successfully";
      })
      .addCase(getMonthlyBalance.rejected, (state, action) => {
